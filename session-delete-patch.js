...
if (!sessionSrc.includes('[PERMANENT SESSION DELETE]')) {
  // Le patch persistence exécuté avant celui-ci remplace fileAuthState par
  // getSessionDir + Mongo auth. On ajoute uniquement ce qui manque.
  ...
  // Les pairings jamais finalisés doivent eux aussi disparaître complètement.
  ...
  const exportAnchor = '  stopSession,\n  toSessionId,';
  if (!sessionSrc.includes(exportAnchor)) throw new Error('[session-delete] export stopSession/toSessionId introuvable');
  sessionSrc = sessionSrc.replace(exportAnchor, '  stopSession,\n  deleteSession,\n  toSessionId,');
