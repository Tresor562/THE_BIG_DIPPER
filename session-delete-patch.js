...
// ── loggedOut : suppression définitive AUTOMATIQUE ────────────────────────
// Un appareil retiré depuis « Appareils connectés » de WhatsApp aboutit au
// chemin terminal loggedOut. Ce cas ne doit jamais rester dans Mongo/index.
// connectionReplaced reste volontairement NON destructif : un autre socket
// peut simplement avoir remplacé l'ancien sans invalider les credentials.
sessionSrc = fs.readFileSync(sessionPath, 'utf8');
if (!sessionSrc.includes('[LOGGED OUT PERMANENT PURGE]')) {
  const anchor = "      sessionIndex.setState(sessionId, { isOnline: false }).catch(() => {});";
  const purgeBlock = `${anchor}\n\n      // [LOGGED OUT PERMANENT PURGE]\n      // Déconnexion volontaire / appareil retiré depuis WhatsApp :\n      // aucune reconnexion et aucune auth persistante ne doit survivre.\n      if (statusCode === DisconnectReason.loggedOut) {\n        console.log(\`[SessionManager] 🗑️ ${'${sessionId}'} loggedOut — suppression persistante définitive\`);\n        try {\n          await deleteSession(phoneNumber, { skipClose: true });\n        } catch (err) {\n          logCriticalSessionError(\`❌ suppression loggedOut ${'${sessionId}'} échouée — ${'${err.message || err}'}\`);\n        }\n        return;\n      }`;
  sessionSrc = replaceOnce(sessionSrc, anchor, purgeBlock, 'purge automatique loggedOut');
  fs.writeFileSync(sessionPath, sessionSrc);
  console.log('[session-delete] purge automatique loggedOut installée');
} else {
  console.log('[session-delete] purge automatique loggedOut déjà installée');
}
...
// Garde-fou explicite : connectionReplaced doit rester terminal mais NON
// destructif. La purge automatique est conditionnée uniquement à loggedOut.
const purgeSectionStart = finalSession.indexOf('[LOGGED OUT PERMANENT PURGE]');
const purgeSection = purgeSectionStart >= 0 ? finalSession.slice(purgeSectionStart, purgeSectionStart + 900) : '';
if (/connectionReplaced[\s\S]{0,300}deleteSession\(/.test(purgeSection)) {
  throw new Error('[session-delete] sécurité: connectionReplaced ne doit pas supprimer les credentials');
}
...
console.log('[session-delete] ✅ loggedOut => suppression définitive ; connectionReplaced conservé');
