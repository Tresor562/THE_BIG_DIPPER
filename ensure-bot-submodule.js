...
// Render doit construire exactement cette révision de DIPPER-, même si le gitlink
// du sous-module du wrapper n'a pas encore été repointé par l'API Contents GitHub.
...
function ensureBotSubmodule() {
  ...
  if (!fs.existsSync(HANDLER)) {
    throw new Error('[submodule] bot/handler.js absent après git submodule update. Render doit avoir accès au dépôt privé Tresor562/DIPPER-.');
  }

  // Le wrapper peut être déclenché par Render alors que son gitlink pointe encore
  // sur une ancienne révision. On récupère et checkout explicitement la révision
  // attendue afin que le build reflète immédiatement les derniers changements.
  console.log(`[submodule] ciblage explicite DIPPER- @ ${TARGET_BOT_SHA.slice(0,12)}...`);
  runGit(['fetch', '--depth=1', 'origin', TARGET_BOT_SHA], 'git fetch révision bot', BOT);
  runGit(['checkout', '--detach', '--force', TARGET_BOT_SHA], 'git checkout révision bot', BOT);
  runGit(['reset', '--hard', TARGET_BOT_SHA], 'git reset révision bot', BOT);
  ...
  if (fullSha !== TARGET_BOT_SHA) {
    throw new Error(`[submodule] mauvaise révision bot après checkout: ${fullSha || 'inconnue'} (attendue ${TARGET_BOT_SHA})`);
  }
