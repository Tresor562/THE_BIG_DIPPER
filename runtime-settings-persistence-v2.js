...
if (!fs.existsSync(BOT)) throw new Error('[runtime-settings-v2] bot/ absent');
...
function replaceOnce(rel, search, replacement, marker, label) {
  ...
  if (marker && src.includes(marker)) {
    console.log(`[runtime-settings-v2] ${label} déjà appliqué`);
    return;
  }
  const count = src.split(search).length - 1;
  if (count !== 1) throw new Error(`[runtime-settings-v2] ${label}: attendu 1 occurrence, trouvé ${count}`);
  src = src.replace(search, replacement);
  write(rel, src);
  console.log(`[runtime-settings-v2] ${label} appliqué`);
}
...
console.log('[runtime-settings-v2] sessionPreferences Mongo durable installé');
...
for (const rel of [
  ...
  if (result.status !== 0) throw new Error(`[runtime-settings-v2] syntaxe ${rel}: ${result.stderr || result.stdout}`);
}

console.log('[runtime-settings-v2] ✅ réglages runtime/groupes + identité/style/menu image persistants Mongo');
